...
## 缺陷 9：WebGL 歌词糊成一坨 / 截断（Hermes 移植回归 · Cursor 已修绘制）

### 现象
改完后中央歌词：字叠成一团、左右被砍成 `t wait…cu`、黄字糊在封面上、上下行/翻译挤在一起。

### 根因（`lyricQuad.ts` drawLyricCanvas）
1. 字号 128px 不换行 → 长英文画出 canvas 被裁  
2. 卡拉 OK 用**中心双向裁剪**而非从左填色 → 像中间截一截  
3. 翻译画在 `y + fontPx*0.85` 侵入下一行 → 三行叠一坨  
4. DOM `LyricLight` 又加了按行反向 parallax，加重错位（已弱化）

### 已做
- 分行 band 布局 + `fitFontPx` 缩字适配宽度  
- 卡拉 OK 改为从文字左缘 LTR clip  
- 翻译独立 band；canvas 加高；DOM 去掉有害行级位移  
...
3. **缺陷 6** — 波浪正态包络  
4. **缺陷 3 / 4** — 若尚未合入  
5. **缺陷 9** — 歌词绘制已由 Cursor 修一版，请回归验收
