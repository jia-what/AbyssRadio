...
ws.onmessage = (ev) => {
  ...
  if (msg.method === 'Runtime.consoleAPICalled') {
    logs.push(msg.params.type + ': ' + msg.params.args.map(a => {
      if (typeof a.value === 'object' && a.value) return JSON.stringify(a.value).slice(0, 150);
      return String(a.value || a.description || '').slice(0, 150);
    }).join(' '));
  }
...
if (diag.length === 0) console.log('(无!)');

// 截图
const shot = await send('Page.captureScreenshot', { format: 'png', fromSurface: true }).catch((e) => ({ error: e.message }));
if (shot.result?.data) {
  fs.writeFileSync('E:\\VM\\AI_audio\\ai-radio-v1\\cdp-shot10.png', Buffer.from(shot.result.data, 'base64'));
  console.log('截图 OK cdp-shot10.png');
}
...
const col = await evalJs(`(async () => {
  try {
    ...
    c2.width = 50; c2.height = 50;
    const ctx2 = c2.getContext('2d');
    ctx2.drawImage(canvas, 0, 0, 50, 50);
    const pts = [[25,25],[10,10],[40,25],[25,40]];
    const out = pts.map(([x,y]) => { const d = ctx2.getImageData(x,y,1,1).data; return [d[0],d[1],d[2]].join(','); });
    return JSON.stringify(out);
  } catch (e) { return 'ERR: ' + e.message.slice(0, 100); }
})()`);
console.log('canvas 颜色:', col);
ws.close();
